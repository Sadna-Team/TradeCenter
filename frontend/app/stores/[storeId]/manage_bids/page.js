...
const ManageBid = () => {
  ...
  const [newBidStoreId, setNewBidStoreId] = useState('');
  const [newBidProductId, setNewBidProductId] = useState('');
  const [newBidProposedPrice, setNewBidProposedPrice] = useState('');
  const [newBidStatus, setNewBidStatus] = useState(null);
  ...
  const handleSaveChanges = () => {
    if (!newBidProposedPrice) {
      setErrorMessage('Please enter a proposed price.');
      return;
    }
    const saveBid = async () => {
      try {
        ...
        setErrorMessage('Failed to add bid');
      }
    };
    saveBid();
    ...
    const editBid = async () => {
      try {
        const response = await api.post('/market/store_worker_counter_bid', {
          store_id,
          bid_id: currentBidId,
          proposed_price: editCounterBid,
        });
        if (response.status !== 200) {
          console.error('Failed to edit bid', response);
          setErrorMessage('Failed to edit bid');
          return;
        }
        fetchBids();
        setErrorMessage('');
